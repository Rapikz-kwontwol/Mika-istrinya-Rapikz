...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    conn.sendMessage(m.chat, { react: { text: "⏳", key: m.key } });
        ...
        if (result.type === 'search') {
            ...
            // Mengambil video 480p dari MP4
            if (!dl.mp4['480p']) throw new Error('Video dengan resolusi 480p tidak tersedia.');
            const videoLink = await dl.mp4['480p']();
            ...
            await conn.sendFile(m.chat, audioLink.url, `${title}.mp3`, '', m, null, {
                mimetype: 'audio/mpeg' // Pastikan dikirim sebagai MP3, bukan voice note
            });
        }
    } catch (e) {

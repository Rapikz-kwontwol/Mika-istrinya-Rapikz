...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    conn.sendMessage(m.chat, { react: { text: "⏳", key: m.key } });
        ...
        if (result.type === 'search') {
            ...
            // Mengambil video 240p dari MP4
            if (!dl.mp4['240p']) throw new Error('Video dengan resolusi 240p tidak tersedia.');
            const videoLink = await dl.mp4['240p']();
...
handler.help = ["ytv240p <link>"];
handler.tags = ["downloader"];
handler.command = /^ytv240p$/i;

...
store?.readFromFile('./bot_store.json')
// save every 10s
setInterval(() => {
	store?.writeToFile('./bot_store.json')
}, 10_000)
...
if(usePairingCode && !conn.authState.creds.registered) {
    ...
    if (RapikzBjir.trim().toLowerCase() !== 'm') {
      ...
      process.exit();
		...
		let phoneNumber = ''
		do {
			phoneNumber = await question(chalk.red('Masukan Nomor Telepon Mu:\n'))
...
async function connectionUpdate(update) {
    ...
    if (connection == 'open') {
        conn.sendMessage(`6283870640516@s.whatsapp.net`, {text: 'BOT AKTIF'})
    }
